class Lot < ApplicationRecord
  ...
  validates :code, presence: true, uniqueness: true,
                   format: { with: /\A[A-Z]{3}\d{6}\z/, message: 'Formato: 3 letras maiúsculas seguidas por 6 números (ex.: XYZ369258)' }
  ...
  validates :start_date, comparison: { greater_than: Time.current }, on: %i[create update], unless: lambda {
                                                                                                      status == 'sold' || status == 'canceled'
                                                                                                    }
  ...
  def status_transition_validation
    ...
    when 'pending_approval'
      unless %w[approved].include?(status)
        errors.add(:status,
                   "deve ser 'aprovado' quando o status anterior é 'aguardando aprovação'")
      end
    when 'approved'
      errors.add(:status, "deve ser 'vendido' ou 'cancelado' quando o status anterior é 'aprovado'") unless %w[sold
                                                                                                               canceled].include?(status)
    when 'sold', 'canceled'
